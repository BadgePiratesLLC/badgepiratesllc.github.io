...
The CVE monitor had detected a fresh vulnerability in our infrastructure overnight. It opened a Nexus ticket. By the time I sat down with coffee, Dinesh — a different agent, with no direct connection to the CVE monitor — had already picked the ticket up, patched the affected script, pushed the fix, and moved the ticket to Done.
...
- **The vocabulary.** Tickets have an `agent` field. ClaudeCode, MasterAI, Dinesh, Skippy, Gilfoyle, Dwight, DrJohnnyFever — each a real assignee. Plus columns: Not started → On Deck → Proceed → In progress → Awaiting Action → Testing → Done.
- **The arc that surprised me.** [PULL: a real Nexus ticket that started in vuln_alerter, got picked up by Dinesh, closed without my involvement.] Show ticket title, the comments thread, who moved it through which columns and when.
...
- A specific ticket arc: vuln_alerter → Dinesh → Done, with timestamps. Probably one of the recent CVE tickets — search for "CVE" in Nexus, find one auto-closed by Dinesh.
...
- Two real risks, both addressed by Nexus's design:
  1. **Agent claims a ticket and does the wrong thing.** Mitigation: every change an agent makes flows through git. Every commit is reviewable, every Nexus column transition is logged with timestamps + comments. If Dinesh ships a bad fix, the ticket stays open in Testing until verified — and the human (me) catches it.
  2. **Agent hits a decision it shouldn't make.** Mitigation: `Awaiting Action` column is the explicit escape hatch. When Dinesh needs my approval to push something risky, ticket lands in Awaiting Action with a clear comment. I see it next time I check the board. No agent is allowed to *decide* on its own — the whole queue is structured so escalation is the path of least resistance.
